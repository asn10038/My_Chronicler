...
public class ChroniclerJExportRunner extends Thread {
    ...
    private static ArrayList<String> serializableLogs = new ArrayList<String>();

    private static ArrayList<String> otherLogs = new ArrayList<String>();
    ...
    /**Generates the test case in the form of a jar file */
    ...
    //ChroniclerJ export runner handles the logs and when the time comes will export the logs
    ...
    //This will export the log that handles objects with the XSTREAM library
    public static void export() {
    	...
    	try {
    		...
    		String xml = "";
    		Log.logLock.lock();
    		//shifts references from current Log to ExportedLog so program can continue to record
            ...
            try {
            	//converts the exported log into xml
            	xml = xstream.toXML(log);
            } catch (Exception ex) {
            	System.err.println("NPE" + ex.getMessage());
            }
            
            ExportedLog.clearLog();//can do this now that the original has been converted to XML
            String name = "chroniclerj_" + System.currentTimeMillis() + ".log";
            otherLogs.add(name);
            //write the xml log to chroniclerj_time_.log
    		...
    		//there is a handler for this in the previous method 
    	}
    	shouldExport = -1; // already exported
    }
    
    private static ExportedSerializableLog logS = new ExportedSerializableLog();
    
    /** Exports the serializable log*/
    ...
    public static void _exportSerializable() {
        if (shouldExportSerializable == -1) {
            ...
            //this is caught in run() and triggers the export stuff
            inst.interrupt();
        }
    }
    	...
    	while(true) {
    		try {
    			...
    			if (shouldExportSerializable == 1)
                 ...
                 if (shouldExportSerializable == 1)
                     exportSerializable();
                 //TODO ask Jon why he exports 2x
                 if (shouldExport == 1)
                     export();
             }
    	}
